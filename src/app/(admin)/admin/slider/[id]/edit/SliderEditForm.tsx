...
export default function SliderEditForm({ slider }: { slider: SliderData }) {
    const router = useRouter();
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [previewImage, setPreviewImage] = useState<string | null>(slider.foto);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert('❌ Gagal memperbarui slider: ' + error);
            }
        } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                <label htmlFor="foto" className="block text-sm font-semibold text-gray-700 mb-2">
                    URL Gambar Slider <span className="text-red-500">*</span>
                </label>
                <input
                    type="text"
                    id="foto"
                    ...
                    placeholder="https://example.com/gambar-slider.jpg"
                />
                <p className="text-xs text-gray-500 mt-1">
                    💡 Masukkan URL gambar yang sudah diupload (format: JPG, PNG, WebP)
                </p>
            </div>
            ...
            <div>
                ...
                <textarea
                    ...
                    name="keterangan"
                    rows={4}
                    defaultValue={slider.keterangan || ''}
            ...
            {slider.tgl_update && (
                <div className="bg-gray-50 rounded-lg p-4">
                    <div className="text-xs text-gray-600">
                        Terakhir diupdate: {new Date(slider.tgl_update).toLocaleString('id-ID', {
                            day: '2-digit',
                            month: 'long',
                            year: 'numeric',
                            hour: '2-digit',
